    ...
    const user = User.findByIdAndUpdate(
        req.user?._id,
        {
            $set: {
                ...
                email: email //can be written as email
            }
        },
        {new: true}
    ).select("-password")

    return res.status(200).json(new ApiResponse(200, user, "Accoutn detail updated successfully"))
...
const updateUserAvatar = asyncHandler( async(req, res) => {
    ...
    if(!avatar.url){
        throw new ApiError(400, "Avatar cloudinary file missing")
    }
...
const updateUserCoverImage = asyncHandler( async(req, res) => {
    ...
    if(!coverImage.url){
        throw new ApiError(400, "CoverImage cloudinary file missing")
    }
